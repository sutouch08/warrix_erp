  ...
  public function index()
  {
    ...
    if(!empty($customers))
    {
      foreach($customers as $rs)
      {
        ...
        $cust->code   = $rs->code;
        $cust->name   = $rs->name;
        $cust->group  = $this->customer_group_model->get_name($rs->group_code);
        $cust->kind   = $this->customer_kind_model->get_name($rs->kind_code);
        $cust->type   = $this->customer_type_model->get_name($rs->type_code);
        $cust->class  = $this->customer_class_model->get_name($rs->class_code);
        $cust->area   = $this->customer_area_model->get_name($rs->area_code);
        $cust->date_upd = $rs->date_upd;
